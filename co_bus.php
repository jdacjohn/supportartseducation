...
if(isset($_GET['bus']))
	...
	if(empty($_GET['bus']))
		...
		{
		$query = "SELECT co_bus.co_bus_name, co_bus.co_bus_desc, card.city_code FROM card, co, co_bus, co_bus_list WHERE '$short'=co_bus.co_bus_id AND co_bus.co_bus_id=co_bus_list.co_bus_id AND co_bus_list.co_id=co.co_id AND '$site_id'=card.site_id AND card.co_id=co.co_id AND card.card_approve='1' AND card.card_video='' LIMIT 1";
		...
		if(mysql_num_rows($result)==0)
			...
			while($row = mysql_fetch_array($result))
				...
				$query = "SELECT COUNT(*) FROM card, co_bus_list WHERE '$site_id'=card.site_id AND '$short'=co_bus_list.co_bus_id AND co_bus_list.co_id=card.co_id AND card.card_approve='1' AND card.card_video=''";
				$limit = '9999';
				echo '<div class="pages" id="top">'; echo pagination($query,$page_link,$limit); echo '</div>'; $limstring = paginationCount($query,$limit);
				echo '<div class="deck">';
				$query2 = "SELECT card.card_id, card.card_front, co.co_id, co.co_name, card.city_code FROM card, co, co_bus_list WHERE '$short'=co_bus_list.co_bus_id AND co_bus_list.co_id=co.co_id AND '$site_id'=site_id AND card.co_id=co.co_id AND card.card_approve='1' AND card.card_video='' ORDER BY card.card_id DESC $limstring";
				...
				while($row = mysql_fetch_array($result))
					...
					$subresult = mysql_query("SELECT card.card_id, card.city_code FROM card, co, co_bus_list WHERE '$short'=co_bus_list.co_bus_id AND co_bus_list.co_id=co.co_id AND '$site_id'=site_id AND '$co_id'=card.co_id AND card.card_approve='1' AND card.card_video!='' LIMIT 1") or die(mysql_error());
		...
		$social_network = '<hr><span style="position:relative;top:4px;margin-right:-1px;margin-top:0px;margin-bottom:0px;padding-bottom:-10px;padding-top:10px;"><a title="'.$co_bus_name.'" href="http://www.facebook.com/sharer.php?s=100&p[url]='.$currentpage.'&p[images][0]=http://supportartseducation.com/assets/img/logo.png&p[title]='.$co_bus_name.'&p[summary]='.$row["co_bus_desc"].'" target="_blank" style=""><img src="http://supportartseducation.com/assets/img/facebook-icon.png" alt="Share on Facebook" style="margin-top:0px;" /></a></span>
